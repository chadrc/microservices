...
CREATE OR REPLACE FUNCTION update_session_with_access_token(token VARCHAR)
  RETURNS TABLE (
    userId INTEGER,
    sessionId INTEGER
  )
    ...
    AS $$
      ...
      BEGIN
        IF EXISTS (SELECT id FROM user_accounts WHERE access_token=token) THEN
          SELECT * FROM update_session_with_access_token(token) INTO found_userId, found_sessionId;
        END IF;
        RETURN QUERY SELECT found_userId, found_sessionId;
      END;
    $$
    ...
    AS $$
      ...
      BEGIN
        IF EXISTS (SELECT id FROM user_accounts WHERE name=username AND password=pass) THEN
          ...
          SELECT * FROM update_session_with_access_token(found_token) INTO found_userId, found_sessionId;
